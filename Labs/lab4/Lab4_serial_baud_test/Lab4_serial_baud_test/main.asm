...
* Inputs:   None			 
* Outputs:  None
* Affected: r16, USARTE0_CTRLB, USARTE0_CTRLC, USARTE0_BAUDCTRLA, USARTE0_BAUDCTRLB
 ***********************************************************************************/
 USART_INIT:
	ldi r16, 0x18
	sts USARTE0_CTRLB, r16		;turn on TXEN and RXEN lines

	ldi r16, 0x23
	sts USARTE0_CTRLC, r16		;Parity = even, 8 bit frame, 1 stop bit

	ldi r16, (BSel & 0xFF)		;select only lower 8 bits of BSel
	sts USARTE0_BAUDCTRLA, r16	;set BAUDCTRLA to lower 8 bits of BSel

	ldi r16, ((BScale << 4) & 0xF0) | ((BSel >> 8) & 0x0F)
	sts USARTE0_BAUDCTRLB, r16	;set BAUDCTRLB to BScale | Bsel.
...
* Inputs:   None			 
* Outputs:  None
* Affected: r16, PORTE_DIR, PORTE_OUT, PORTQ_DIR, PORTQ_OUT
 ***********************************************************************************/
 GPIO_INIT:
	ldi r16, 0x08
	sts PORTE_DIRSET, r16		;set PORTE_PIN3 as output for TX pin of USARTE0
	sts PORTE_OUTSET, r16		;set the TX line to default to '1'

	ldi r16, 0x04
	sts PORTE_DIRCLR, r16		;set RX pin for input
...
* Affected: USARTE0_STATUS, USARTE0_DATA
 ***********************************************************************************/
OUT_CHAR:
	push r17
